...
public class PaintTestFirstNextComb
extends Panel
implements Testlet {
    public static int DELAY_AMOUNT = 250;
    public static int SMALL_DELAY_AMOUNT = 1000;
    ...
    private void nextCard(Robot robot, CardLayout layout2, int card) {
        for (int i = 0; i < card; ++i) {
            robot.waitForIdle();
            robot.delay(SMALL_DELAY_AMOUNT);
            layout2.next(this);
        }
    }
    ...
    private void doTest(TestHarness harness, Robot robot, Frame frame, Component component2, Color componentColor) {
        ...
        harness.check(this.getColorForTopBorder(robot, frame, component2), componentColor);
        harness.check(this.getColorForBottomBorder(robot, frame, component2), componentColor);
    }
