...
public class WrappedWithOffsetBufferTest {
    ...
    private void intialState(TestHarness h, BufferFactory factory) {
        ...
        this.checkStatus(h, buf, "intialState", 20, 20, true, 10, 10);
    }
    ...
    private void position(TestHarness h, BufferFactory factory) {
        ...
        this.checkStatus(h, buf, "position", 20, 20, true, 19, 1);
        buf.position(20);
        this.checkStatus(h, buf, "position", 20, 20, false, 0, 20);
        buf = factory.newInstance();
        try {
            buf.position(-1);
    ...
    private void mark(TestHarness h, BufferFactory factory) {
        ...
        this.checkStatus(h, buf, "mark", 20, 20, true, 10, 10);
        buf.position(15);
        this.checkStatus(h, buf, "mark", 20, 20, true, 5, 15);
        h.check(buf.reset(), buf, "mark: buf.reset()");
        this.checkStatus(h, buf, "mark", 20, 20, true, 10, 10);
        buf.position(16);
        this.checkStatus(h, buf, "mark", 20, 20, true, 4, 16);
        buf.reset();
        this.checkStatus(h, buf, "mark", 20, 20, true, 10, 10);
        buf = factory.newInstance();
        buf.position(5);
        buf.mark();
        this.checkStatus(h, buf, "mark", 20, 20, true, 15, 5);
        buf.position(6);
        this.checkStatus(h, buf, "mark", 20, 20, true, 14, 6);
        buf.reset();
        this.checkStatus(h, buf, "mark", 20, 20, true, 15, 5);
        buf.position(7);
        this.checkStatus(h, buf, "mark", 20, 20, true, 13, 7);
        buf.reset();
        this.checkStatus(h, buf, "mark", 20, 20, true, 15, 5);
        buf = factory.newInstance();
        buf.position(5);
        ...
        this.checkStatus(h, buf, "mark", 20, 20, true, 16, 4);
    }
    ...
    private void limit(TestHarness h, BufferFactory factory) {
        ...
        this.checkStatus(h, buf, "limit", 20, 4, true, 1, 3);
        buf.reset();
        this.checkStatus(h, buf, "limit", 20, 4, true, 2, 2);
        buf = factory.newInstance();
        buf.position(5);
        buf.mark();
        buf.position(6);
        buf.limit(4);
        this.checkStatus(h, buf, "limit", 20, 4, false, 0, 4);
        try {
            buf.reset();
    ...
    private void rewind(TestHarness h, BufferFactory factory) {
        ...
        this.checkStatus(h, buf, "rewind", 20, 9, true, 9, 0);
        try {
            buf.reset();
    ...
    private void clear(TestHarness h, BufferFactory factory) {
        ...
        this.checkStatus(h, buf, "clear", 20, 20, true, 20, 0);
        try {
            buf.reset();
    ...
    private void flip(TestHarness h, BufferFactory factory) {
        ...
        this.checkStatus(h, buf, "flip", 20, 6, true, 6, 0);
        try {
            buf.reset();
