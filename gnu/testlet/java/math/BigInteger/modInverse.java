...
implements Testlet {
    ...
    private void testCase(TestHarness harness, int i, String ps, String qs) {
        ...
        try {
            this.p = new BigInteger(ps, 16);
            this.q = new BigInteger(qs, 16);
            harness.check(this.p.isProbablePrime(50), "p[" + i + "] is probable prime");
            harness.check(this.p.gcd(e).equals(BigInteger.ONE), "gcd(p[" + i + "], e) == 1");
            harness.check(this.q.isProbablePrime(50), "q[" + i + "] is probable prime");
            harness.check(this.q.gcd(e).equals(BigInteger.ONE), "gcd(q[" + i + "], e) == 1");
            this.phi = this.p.subtract(BigInteger.ONE).multiply(this.q.subtract(BigInteger.ONE));
            e.modInverse(this.phi);
        }
