...
public strictfp class sin_strictfp
implements Testlet {
    ...
    private void testInputValues(TestHarness harness) {
        for (int i = 0; i < inputValues.length; ++i) {
            double res = sin_strictfp.testedFunction(inputValues[i]);
    ...
    private void testNaN(TestHarness harness) {
        for (int i = 0; i < NaNValues.length; ++i) {
            ...
            harness.check(Double.doubleToRawLongBits(sin_strictfp.testedFunction(valNaN)), bitsNaN);
        }
    }
    ...
    public static void main(String[] argv) {
        for (int i = 0; i < inputValues.length; ++i) {
            ...
            double output = sin_strictfp.testedFunction(inputValues[i]);
