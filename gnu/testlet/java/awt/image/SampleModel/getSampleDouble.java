...
implements Testlet {
    ...
    public void test(TestHarness harness) {
        ...
        harness.check(m.getSampleDouble(1, 2, 0, db), 0.0);
        harness.check(m.getSampleDouble(1, 2, 1, db), 0.0);
        harness.check(m.getSampleDouble(1, 2, 2, db), 0.0);
        ((SampleModel)m).setPixel(1, 2, new int[]{1, 2, 3}, db);
        harness.check(m.getSampleDouble(1, 2, 0, db), 1.0);
        harness.check(m.getSampleDouble(1, 2, 1, db), 2.0);
        harness.check(m.getSampleDouble(1, 2, 2, db), 3.0);
        boolean pass = false;
        try {
