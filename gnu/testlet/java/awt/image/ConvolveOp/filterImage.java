...
implements Testlet {
    ...
    public void test(TestHarness harness) {
        ...
        this.testFilter(harness);
        this.testPremultiplied(harness);
    }
    ...
    private void testDefaults(TestHarness harness) {
        ...
        Kernel k1 = new Kernel(3, 3, new float[]{0.1f, 0.2f, 0.3f, 0.4f, 0.5f, 0.6f, 0.7f, 0.8f, 0.9f});
        ConvolveOp op = new ConvolveOp(k1, 0, null);
        try {
            op.filter(img, img);
            harness.check(false);
        }
    ...
    private void testFilter(TestHarness harness) {
        ...
        Kernel k1 = new Kernel(3, 3, new float[]{0.2f, 0.3f, 0.5f, 0.4f, 0.6f, 0.7f, 0.6f, 0.9f, 0.8f});
        ConvolveOp op = new ConvolveOp(k1, 0, null);
        ...
        harness.check(dest.getSample(2, 2, 3) - 39 >= 0 && dest.getSample(2, 2, 3) - 39 <= 1);
        ...
        harness.check(dest.getSample(2, 3, 3) - 119 >= 0 && dest.getSample(2, 3, 3) - 119 <= 1);
    ...
    private void testPremultiplied(TestHarness harness) {
        ...
        BufferedImage img = new BufferedImage(5, 5, 3);
        WritableRaster r = img.getRaster();
        for (int i = 0; i < r.getHeight(); ++i) {
            for (int j = 0; j < r.getWidth(); ++j) {
                r.setSample(j, i, 0, i * 5 + j * 8);
