...
implements Testlet {
    ...
    void testWithoutChildren(TestHarness h) {
        ...
        h.check(l.maximumLayoutSize((Container)c).width, 0);
        h.check(l.maximumLayoutSize((Container)c).height, 0);
    }
    ...
    void testWithOneChild(TestHarness h) {
        ...
        c.add(c1);
        OverlayLayout l = new OverlayLayout(c);
        h.check(l.maximumLayoutSize((Container)c).width, 30);
        h.check(l.maximumLayoutSize((Container)c).height, 30);
    }
    ...
    void testWithTwoChildren(TestHarness h) {
        ...
        c.add(c2);
        OverlayLayout l = new OverlayLayout(c);
        h.check(l.maximumLayoutSize((Container)c).width, 60);
        h.check(l.maximumLayoutSize((Container)c).height, 60);
    }
    ...
    void testWithAlignment(TestHarness h) {
        ...
        c.add(c3);
        OverlayLayout l = new OverlayLayout(c);
        h.check(l.maximumLayoutSize((Container)c).width, 90);
        h.check(l.maximumLayoutSize((Container)c).height, 60);
    }
    ...
    void testWrongContainer(TestHarness h) {
        ...
        try {
            h.check(l.maximumLayoutSize((Container)c2).width, 30);
            h.fail("maximumLayoutSize must throw AWTError when called with wrong container.");
        }
        catch (AWTError ex) {
            h.check(true);
        }
